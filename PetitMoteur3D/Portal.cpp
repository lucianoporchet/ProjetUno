...
Portal::Portal(const std::string& nomfichier, PM3D::CDispositifD3D11* _pDispositif, float scale, PxVec3 pos)
	: CObjetMesh(nomfichier, _pDispositif, scale), scale{ scale }
{
	// Il s'agit ici du portail final : scene 3, forcement.
	body = PhysXManager::get().createDynamic(PxTransform(pos), PxSphereGeometry(scale), PxVec3(0, 0, 0), 3);
	...
	PhysXManager::get().setupFiltering(body, FilterGroup::eFinalPortal, FilterGroup::ePlayer);
}
